...
function maxSpeedRide(position) {
    let maxSpeed = 0
    position.forEach(position => {
        if (position != null && position.speed > maxSpeed)
            maxSpeed = position.speed
    });
    return (maxSpeed * 3.6).toFixed()
}
...
function getDistance(positions) {
    ...
    for (let i = 0; i < positions.length - 1; i++) {
        ...
        const deltaLongititude = toRad(p2.longitude - p1.longitude);

        const a = Math.sin(deltaLatitude / 2)
            * Math.sin(deltaLatitude / 2)
            + Math.sin(deltaLongititude / 2)
            * Math.sin(deltaLongititude / 2)
...
function getRideDate(ride){
    ...
    const minutes = d.toLocaleString('en-US', {minute: '2-digit'})
